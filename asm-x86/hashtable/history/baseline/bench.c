...
static volatile uint64_t g_lookup_sink = 0;
...
static uint64_t run_insert_phase(ht_t *ht, loaded_file_t *file) {
	size_t i = 0;
	...
	while (i < file->size) {
		...
		if (i > start) {
			(void)ht_increment(ht, file->data + start, i - start, 1);
			++token_count;
		}
	}
...
static uint64_t run_lookup_phase(const ht_t *ht, loaded_file_t *file) {
	size_t i = 0;
	uint64_t token_count = 0;
	uint64_t sink = 0;
	...
	while (i < file->size) {
		...
		if (i > start) {
			sink ^= ht_lookup(ht, file->data + start, i - start);
			++token_count;
		}
	}
...
static void print_most_frequent(const ht_view_t *view) {
	fputs("most_frequent: ", stdout);
	if (view->key.ptr == NULL) {
		fputs("<none>\n", stdout);
		return;
	}

	(void)fwrite(view->key.ptr, 1, view->key.len, stdout);
	printf(" (%lu)\n", view->count);
}
...
int main(int argc, char **argv) {
	...
	ht = ht_create();
	if (ht == NULL) {
		die_msg("ht_create failed");
	}

	CALLGRIND_START_INSTRUMENTATION;

	sample_begin(&insert_sample);
	insert_count = run_insert_phase(ht, &insert_file);
	sample_end(&insert_sample);

	sample_begin(&lookup_sample);
	lookup_count = run_lookup_phase(ht, &lookup_file);
	sample_end(&lookup_sample);

	sample_begin(&max_sample);
	most = ht_most_frequent(ht);
	sample_end(&max_sample);

	CALLGRIND_STOP_INSTRUMENTATION;

	printf("inserts: %lu\n", insert_count);
	printf("lookups: %lu\n", lookup_count);
	print_most_frequent(&most);
	print_phase("insert", &insert_sample);
	print_phase("lookup", &lookup_sample);
	print_phase("max",    &max_sample);
	
	printf("Effective load: %lf (target %lf)\n", ht_load_factor(ht), LOAD_FACTOR);
