/*
 * ht.c v2
 * minimize arena touches by storing prefixes
 */
...
typedef struct ht_entry {
	size_t   next;     /* 0 = end of chain */
	size_t   key_off;  /* offset into arena */
	size_t   key_len;  /* in bytes */
	uint64_t hash;
	uint64_t count;    /* occurrence count */
	uint64_t prefix8;  /* first up to 8 key bytes, zero-padded */
} ht_entry_t;

struct ht {
	size_t     *buckets;     /* bucket heads, 0 = empty, indices are 1-based */
	size_t      bucket_count;
...
static int rehash(ht_t *ht, size_t new_bucket_count) {
	size_t *new_buckets = calloc(new_bucket_count, sizeof(size_t));
	if (!new_buckets) {
		return 0;
	}
...
ht_t *ht_create() {
	...
	ht->buckets =     (size_t *)calloc(ht->bucket_count, sizeof(size_t));
...
void ht_clear(ht_t *ht) {
	...
	if (ht->buckets && ht->bucket_count) {
		memset(ht->buckets, 0, ht->bucket_count * sizeof(size_t));
	}
...
uint64_t ht_increment(ht_t *ht, const void *key, size_t key_len, uint64_t delta) {
	...
	for (size_t i = ht->buckets[bucket]; i != 0; i = ht->entries[i].next) {
		ht_entry_t *e = &ht->entries[i];

		if (e->hash == hash && e->key_len == key_len && e->prefix8 == prefix8 &&
		    (key_len <= 8 || memcmp(ht->arena + e->key_off, key, key_len) == 0)
		) {
			e->count += delta;
			return e->count;
		}
