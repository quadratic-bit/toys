...
struct ReactorState : public State {
    ...
    ReactorState(Window *w)
        : State(w), wall_speed(0), wall_speed_changed(false),
        add_particle(false), delete_particle(false), reactor(NULL) {}

    ReactorState(Window *w, Time now_)
        : State(now_, w), wall_speed(0),
        wall_speed_changed(false), add_particle(false),
